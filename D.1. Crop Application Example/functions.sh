...
function add_item {
    ...
    jq -e \
        --arg item ${item} \
        '.links += [{ "type":"application/geo+json", "href":$item, "rel": "item"}]' ${catalog} > ${catalog}.tmp && mv ${catalog}.tmp ${catalog}
}
export -f add_item
...
function init_item {
    ...
    jq --argjson dt "${datetime}" '.properties.datetime=$dt' | # set the datetime
...
function add_asset {
    ...
    jq -e -r \
        ...
        --arg title "${title}" \
        --arg role "${role}" \
        '.assets += { ($asset_key) : { "role":[$role], "href":$href, "type":$type, "title":$title}}' ${item} > ${item}.tmp && mv ${item}.tmp ${item}
}
export -f add_asset
